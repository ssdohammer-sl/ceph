...
const double DEFAULT_SAMPLING_RATIO = 100;
...
void DedupProcessor::initialize()
{
  // reserve DedupWorkers
  workers.reserve(num_workers);
  for (int i = 0; i < num_workers; ++i) {
    auto worker = std::make_unique<DedupWorker>(dpp, cct, i, this);
 ...
 */
//int DedupProcessor::process(const rgw_bucket_dir_entry obj)
int DedupProcessor::process(rgw::sal::Object* obj)
{
  int ret = 0;

  if (fp_algo != "sha1" && fp_algo != "sha256" && fp_algo != "sha512") {
    cerr << "invalid fingerprint algorithm" << std::endl;
    return -1;
  }

  bufferlist obj_data;
  ret = read_object(obj, obj_data);
  if (ret < 0) {
    cerr << "failed to read " << obj->get_name() << std::endl;
    return ret;
  }

  if (is_hot_object(obj)) {
    return ret;
  }

  // chunking
  vector<tuple<bufferlist, ChunkInfo>> total_chunk_info;
  ret = do_chunking(obj_data, total_chunk_info);
  if (ret < 0) {
     cerr << "total size of all chunks is different from object data length" << std::endl;   
  }

  // calc dedup ratio
  //size_t duplicated_size = 0;
  //list<chunk_t> redundant_chunks;
  for (auto& chunk : total_chunk_info) {
    auto& chunk_data = std::get<0>(chunk);
    string fingerprint = generate_fingerprint(chunk_data);
    ChunkInfo chunk_boundary = std::get<1>(chunk);

    // TODO: check how much duplicated chunk is 
    // update FpStore
...
int DedupProcessor::get_buckets()
{
  void* handle = nullptr;
  bool truncated = true;
  int ret = 0;

  std::cout << "get_buckets start start: " << store << std::endl;
  ...
  std::cout << "meta_list_keys_init done" << std::endl;
  while (truncated) {
...
int DedupProcessor::get_objects()
{
  if (buckets.empty()) {
    ldout(cct, 0) << __func__ << " no available buckets" << dendl;
    return -1;
  }
    ...
    while (truncated) {
      ...
      for (auto obj : results.objs) {
        ...
        if (!is_contain) {
          ...
          append_obj(bkt->get_object(obj.key).get());
        }
      }
...
void* DedupProcessor::entry()
{
  while (!down_flag)
  {
    assert(fp_algo != "sha1" && fp_algo != "sha256" && fp_algo != "sha512");
    assert(chunk_algo != "fastcdc" && chunk_algo != "fixed");
    ...
    auto sampled_indexes = sample_object_index();
    ...
    vector<unique_ptr<DedupWorker>>::iterator it = workers.begin();
    for (auto idx : sampled_indexes) {
      // fixit
      (*it)->append_obj(objects[idx]);
      (*it)->append_sal_obj(objs[idx]);
...
bool DedupProcessor::going_down()
{
  return down_flag;
}

int DedupProcessor::set_sampling_ratio(int new_sampling_ratio)
{
...
vector<size_t> DedupProcessor::sample_object_index()
{
...
int DedupProcessor::read_object(rgw::sal::Object* obj, bufferlist& obj_data)
{
  int ret = 0;
...
int DedupProcessor::set_chunk_size(uint64_t new_chunk_size)
{
  if (new_chunk_size > 0) {
...
int DedupProcessor::set_chunk_algo(string new_chunk_algo)
{
  if (new_chunk_algo == "fastcdc" || new_chunk_algo == "fixed") {
...
int DedupProcessor::do_chunking(bufferlist obj_data,
...
int DedupProcessor::set_fp_algo(string new_fp_algo)
{
  if (new_fp_algo == "sha1" || new_fp_algo == "sha256" || new_fp_algo == "sha512") {
    fp_algo = new_fp_algo;
    return 0;
  }
  return -1;
...
string DedupProcessor::generate_fingerprint(bufferlist chunk_data)
{
  if (fp_algo == "sha1") {
    ...
    assert(0 == "unrecognized fingerprint algorithm");
  }
  return "";
}

bool DedupProcessor::is_hot_object(rgw::sal::Object* obj)
{
  bool ret = false;
  
  // TODO: need to decide the object is hot or cold

  return ret;
}

void* DedupWorker::entry()
{
